...
func SetupRouter(container *di.Container) *gin.Engine {
	...
	checkoutGroup := r.Group("/kitchencontrol/api/v1/checkouts")
	{
		...
		checkoutGroup.PUT("/confirmtion/payment", container.WebhookCheckoutController.PutConfirmPayment)   // verifica se determinada ordem foi paga
	}

	// Kitchen routes
	kitchenGroup := r.Group("/kitchencontrol/api/v1/kitchens")
	{
		kitchenGroup.GET("/orders/paid", container.FindKitchenAllController.GetAllOrdersInKitchen)
	}
